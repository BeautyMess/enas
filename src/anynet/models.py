...
class Model(object):
		...
		with tf.device("/cpu:0"):
			...
			def _pre_process(x):
				...
				if self.cutout_size is not None:
					...
					start = tf.random_uniform([2], minval=0, maxval=w_or_h, dtype=tf.int32)
					mask = tf.pad(mask, [[self.cutout_size + start[0], w_or_h - start[0]],
						[self.cutout_size + start[1], w_or_h - start[1]]])
					mask = mask[self.cutout_size: self.cutout_size + w_or_h,
						self.cutout_size: self.cutout_size + w_or_h]
					mask = tf.reshape(mask, [w_or_h, w_or_h, 1])
					mask = tf.tile(mask, [1, 1, inp_C])
	...
	def eval_once(self, sess, eval_set, feed_dict=None, verbose=False):
		...
		if verbose:
			print ""
		print "{}_accuracy: {:<6.4f}".format(eval_set, float(total_acc) / total_exp)
